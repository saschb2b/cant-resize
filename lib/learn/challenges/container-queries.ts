...
@container (min-width: 250px) {
  .sidebar .widget {
    ...
    correctSide: "right",
    explanationCorrect:
      "`@container` queries only work when an ancestor has `container-type` set. Without it, the query has no container to measure and the styles won't apply. `inline-size` is the most common value — it tracks the container's width.",
    explanationWrong:
...
@container sidebar (min-width: 300px) {
  .card { flex-direction: row; }
    ...
    explanationWrong:
      "Without naming, `@container` matches the nearest container ancestor. If the `.card` moves from the sidebar to the page, it suddenly queries the page's width instead — a subtle bug that's hard to debug because the CSS didn't change.",
    sourceUrl:
...
.hero-title {
  ...
  return (
    ...
    explanationWrong:
      "`ResizeObserver` triggers a state update and re-render on every resize frame. This causes layout thrashing — the browser calculates layout, JavaScript reads it, updates state, React re-renders, browser recalculates layout. Container queries handle this entirely in CSS.",
    sourceUrl:
      "https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_containment/Container_queries",
    sourceLabel: "MDN: Container queries",
  },
